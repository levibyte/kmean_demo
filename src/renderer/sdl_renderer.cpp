...
LeSdlRenderer::LeSdlRenderer(){ //const char* title) {
    ...
    m_kx = 1;
    m_ky = 1;
    SDL_RenderSetScale(m_render,6.0,6.0);
    //assert(0);
}
...
int LeSdlRenderer::t_x(int x) {
    return m_kx*x+m_dx;
    
}

int LeSdlRenderer::t_y(int y) {
    return m_ky*y+m_dy;
}
...
void LeSdlRenderer::enter_event_loop() {
	...
	while( !quit ) {
		...
		if (currentTime > lastTime + 10) {
                    ...
                    LeSdlRenderer* m_renderer_controller = this;
                    while (SDL_PollEvent( &e )) {
				if ( m_renderer_controller ) {
					if ( e.type == SDL_QUIT ) quit = true;
#ifndef OS_ANDROID					
                                        else if ( e.type == SDL_MOUSEBUTTONDOWN && e.button.button == SDL_BUTTON_LEFT )  m_renderer_controller->on_mouse_click(1);
					else if ( e.type == SDL_MOUSEBUTTONDOWN && e.button.button == SDL_BUTTON_RIGHT ) m_renderer_controller->on_mouse_click(0);
					else if ( e.type == SDL_MOUSEMOTION  ) {
						last_x = e.motion.x;
						last_y = e.motion.y;
						m_renderer_controller->on_mouse_move(last_x,last_y);
					}
					else if ( e.type == SDL_KEYDOWN ) { m_renderer_controller->on_key_press(int(e.key.keysym.sym)); }
#endif					
                        ...
                        m_renderer_controller->on_mouse_move(e.tfinger.x * SCREEN_WIDTH,e.tfinger.y * SCREEN_HEIGHT);
                    }
                }
			}
			//SDL_Log("hatuk");
			...
			if(m_renderer_controller) {
				...
				m_renderer_controller->on_update_event(currentTime);
				m_renderer_controller->on_draw_event();
				scene_draw();
			}
